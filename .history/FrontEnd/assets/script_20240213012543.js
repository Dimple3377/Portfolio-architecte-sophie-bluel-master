...
async function recoverWork(workId) {
  console.log(`Tentative de récupération du travail avec ID: ${workId}`);
  try {
    const response = await fetch(
      `http://localhost:5678/api/works/recover/${workId}`,
      {
        method: "POST",
        headers: {
          Authorization: "Bearer " + sessionStorage.getItem("token"),
    ...
    if (response.ok) {
      console.log("Travail récupéré avec succès");
      await loadWork(); // Recharger les travaux pour afficher la photo récupérée
      displayWork(); // Afficher les travaux mis à jour
    } else {
      console.error("Erreur lors de la tentative de récupération");
    }
  } catch (error) {
    console.error(
      "Erreur lors de l'appel à l'API pour la récupération:",
      error
    );
  }
}
...
function displayWorkInModal() {
  ...
  works.forEach((work) => {
    ...
    // Supposons que 'work.isDeleted' est une propriété pour vérifier si le travail a été supprimé
    if (work.isDeleted) {
      const recoverButton = document.createElement("button");
      recoverButton.innerText = "Récupérer";
      recoverButton.onclick = () => recoverWork(work.id);
      workFigure.appendChild(recoverButton);
    }
