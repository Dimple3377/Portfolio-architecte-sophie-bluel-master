...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (sessionStorage.getItem("token")) {
    // Modification pour changer "Login" en "Logout"
    if (sessionStorage.getItem("token")) {
      // Si l'utilisateur est connecté (token présent dans sessionStorage)
      document.getElementById("loginLogoutButton").textContent = "Logout";
      document.getElementById("loginLogoutButton").href = "#"; // Ajustez selon votre implémentation de déconnexion

      // Optionnel : Ajouter un gestionnaire d'événements pour le processus de déconnexion
      document
        .getElementById("loginLogoutButton")
        .addEventListener("click", (event) => {
          event.preventDefault(); // Empêcher le lien de naviguer
          sessionStorage.removeItem("token"); // Supprimer le token de la session
          window.location.href = "index.html"; // Rediriger vers la page d'accueil ou de connexion
        });
    } else {
      // S'assurer que le lien est correct pour les utilisateurs non connectés
      document.getElementById("loginLogoutButton").textContent = "Login";
      document.getElementById("loginLogoutButton").href = "login.html";
    }
