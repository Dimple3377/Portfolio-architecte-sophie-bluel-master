...
function initializeModal() {
  ...
  function adjustLoginLogoutButton() {
    const loginLogoutButton = document.getElementById("loginLogoutButton"); // Assurez-vous que cet ID correspond à votre bouton dans le HTML
    if (!loginLogoutButton) {
      console.error("Bouton de connexion/déconnexion non trouvé.");
      return;
    }
    if (sessionStorage.getItem("token")) {
      ...
      loginLogoutButton.onclick = function () {
        ...
        window.location.href = "login.html"; // Modifiez selon votre structure de pages
      };
    } else {
      loginLogoutButton.textContent = "Login";
      // Assurez-vous que cet URL est correct et mène à votre page de connexion
      loginLogoutButton.href = "login.html";
      loginLogoutButton.onclick = null; // Supprime le gestionnaire d'événements précédent s'il existe
    }
  }
